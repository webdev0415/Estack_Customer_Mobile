...
function* getUserDetails(payload) {
  ...
  try {
    ...
    const { data } = yield call(
      api.get,
      `${endpoints.USER_DETAILS_ENDPOINT}/${_id}`,
      Authorization(accessToken)
    );
...
function* changePassword({ payload }) {
  ...
  try {
    const { data } = yield call(
      api.patch,
      endpoints.USER_CHANGE_PASSWORD_ENDPOINT,
      payload,
      Authorization(accessToken)
    );
    yield all([
    ...
    if (error.request.status === 403) alert("Wrong current password!");
...
function* updateUserDetails({ payload }) {
  ...
  try {
    const { data } = yield call(
      api.patch,
      endpoints.USER_DETAILS_ENDPOINT,
      payload,
      Authorization(accessToken)
    );
    yield put(actions.setUserAction(data));
    ...
    if (error.request.status === 401)
      yield updateAccessTokenByRefresh({
        next: updateUserDetails,
        pl: payload
      });
  }
}
...
function* getAvailableShops() {
  ...
  try {
    const { data } = yield call(
      api.get,
      endpoints.GET_AVAILABLE_BUSINESSES_ENDPOINT,
      Authorization(accessToken)
    );
...
function* getCurrentStoreInfo({ payload }) {
  ...
  try {
    const { data } = yield call(
      api.get,
      `${endpoints.GET_CURRENT_BUSINESS_ENDPOINT}${_id}`,
      Authorization(accessToken)
    );
...
function* joinUserToStore({ payload }) {
  ...
  try {
    yield call(
      api.put,
      `${endpoints.JOIN_USER_TO_STORE_ENDPOINT}${_id}`,
      {},
      Authorization(accessToken)
    );
    const { data } = yield call(
    ...
    yield all([getMyMemberShips(), getAvailableShops()]);
  } catch (error) {
...
function* getMyMemberShips() {
  ...
  try {
    const { data } = yield call(
      api.get,
      endpoints.GET_MY_MEMBERSHIPS_ENDPOINT,
      Authorization(accessToken)
    );
...
function* getTransactions({ payload }) {
  ...
  try {
    ...
    const { data } = yield call(
      api.get,
      `${endpoints.TRANSACTIONS_LOG_LIST_ENDPOINT}?${start}${end}${typeCase}`,
      Authorization(accessToken)
    );
...
function* getWalletData() {
  ...
  try {
    const { data } = yield call(
      api.get,
      endpoints.CUSTOMER_WALLET_DATA_ENDPOINT,
      Authorization(accessToken)
    );
...
function* getTransactionTotals() {
  ...
  try {
    const { data } = yield call(
      api.get,
      endpoints.TRANSACTION_TOTALS_ENDPOINT,
      Authorization(accessToken)
    );
...
function* getCoupons({ payload }) {
  ...
  try {
    ...
    const { data: coupons } = yield call(
      api.get,
      `${endpoints.GET_COUPONS_LIST_ENDPOINT}${businessId}`,
      Authorization(accessToken)
    );
...
function* createCoupon({ payload }) {
  ...
  try {
    ...
    yield call(
      api.post,
      `${endpoints.CREATE_COUPON_ENDPOINT}${businessId}`,
      { cost },
      Authorization(accessToken)
    ),
    ...
    if (error.request.status === 404) {
      Alert.alert(
        "Failed",
        "Looks like business is not active now. Try again later("
      );
    }
  }
}
...
function* redeemCoupon({ payload }) {
  ...
  try {
    ...
    const { data } = yield call(
      api.put,
      `${endpoints.REDEEM_COUPON_ENDPOINT}${_id}`,
      coords,
      Authorization(accessToken)
    );
...
function* sendEmail({ payload }) {
  try {
    ...
    const { data } = yield call(api.post, endpoints.GET_PASSWORD_ENDPOINT, {
      email
    });
    ...
    if (data === true) {
      ...
      if (cb)
        yield put(StackActions.push({ routeName: "Otp", params: { email } }));
    } else {
      yield put(actions.turnOffSpinnerAction());
      setTimeout(
        () =>
          Alert.alert(
            "Ooops...",
            "Can not find your email. Check it, please, and try again."
          ),
        0
      );
    }
...
function* uploadImage({ payload }) {
  ...
  try {
    ...
    const { data } = yield call(
      api.post,
      endpoints.UPLOAD_IMAGE_ENDPOINT,
      formData,
      Authorization(accessToken)
    );
  ...
  try {
    if (nots === true) {
      if (payload.type === "coupon") {
        const { data } = yield call(
          api.get,
          `${endpoints.GET_CURRENT_BUSINESS_ENDPOINT}${payload.coupon.businessId}`,
          Authorization(accessToken)
        );
        sendLocalNotification({
          title: "Coupon approved",
          text: `You have successfully redeemed coupon at ${data.business.brandName}`
        });
      } else {

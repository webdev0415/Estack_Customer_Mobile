...
function* signIn({ payload }) {
  try {
    ...
    if (error.request.status === 404) setTimeout(() => Alert.alert('Ooops!', 'User with such email was not found'));
  }
}
...
function* googleSignIn({ payload }) {
  try {
    const { data } = yield call(api.post, endpoints.GOOGLE_SIGN_IN_ENDPOINT, payload);
...
function* googleSignUp({ payload }) {
  try {
    ...
    const { data } = yield call(api.post, `${endpoints.GOOGLE_SIGN_UP_ENDPOINT}${token}`, { fullName } );
    setTimeout(() => alert('Success'), 0);
...
function* googleAuth({ payload }) {
  ...
  try {
    ...
    if (error.request.status === 400) {
      setTimeout(() => Alert.alert('Failed', 'Google creds is not valid or google services is unavailable'), 0);
      yield put(actions.turnOffSpinnerAction());
    }
    if (error.request.status === 409) {
      yield GoogleSignin.revokeAccess();
    ...
    if (error.request.status === 404) {
      try {
        const { data } = yield call(api.post, `${endpoints.GOOGLE_SIGN_UP_ENDPOINT}${payload.token}`, { fullName: payload.fullName } );
...
function* facebookSignIn({ payload }) {
  try {
    yield put(actions.turnOnSpinnerAction());
    const { data } = yield call(api.post, endpoints.FACEBOOK_SIGN_IN_ENDPOIT, payload);
    yield all([
      AsyncStorage.setItem('accessToken', data.accessToken),
      AsyncStorage.setItem('refreshToken', data.refreshToken),
      put(actions.saveTokenAction(data)),
    ...
    if (error.request.status === 401) setTimeout(() => Alert.alert('Hm...', 'Some conflicts with your google account. Try to login.'), 0);
    if (error.request.status === 404) setTimeout(() => Alert.alert('Ooops!', 'User with such email was not found'));
...
function* facebookSignUp({ payload }) {
  try {
    ...
    const { data } = yield call(api.post, `${endpoints.FACEBOOK_SIGN_UP_ENDPOINT}${token}`, { fullName } );
    ...
    if (error.request.status === 401) setTimeout(() => Alert.alert('Hm...', 'Some conflicts with your facebook account. Try to login.'), 0);
    if (error.request.status === 404) setTimeout(() => Alert.alert('Ooops...', 'Please, add email to your facebook account.'), 0);
    if (error.request.status === 409) setTimeout(() => alert('This email exists already'));
  }
};
...
function* getVerify({ payload }) {
  try {
    ...
    yield put(StackActions.push({routeName: 'RegisterOtp', params: { email: payload.email, fullName: payload.fullName }}))
  } catch (error) {
      if (error.request.status === 409) setTimeout(() => Alert.alert('Failed', 'This email is already exists'), 100)
  } finally {
    yield put(actions.turnOffSpinnerAction());
  }
};
...
function* verify({ payload }) {
  try {
    ...
    if (data) {
      yield put(actions.signUpAction({ code: data, email: payload.email, fullName: payload.fullName }));
    }
    console.log(data);
  } catch (error) {
    yield put(actions.turnOffSpinnerAction());
...
function* appleSignUp({ payload }) {
  try {
    yield put(actions.turnOffSpinnerAction());
    const { data } = yield call(api.post, `${endpoints.APPLE_SIGN_UP_ENDPOINT}${payload.identityToken}`, { fullName: `${payload.fullName.givenName} ${payload.fullName.familyName}` });
    yield all([
      AsyncStorage.setItem('accessToken', data.accessToken),
      AsyncStorage.setItem('refreshToken', data.refreshToken),
      put(actions.saveTokenAction(data)),
    ...
    if (error.request.status === 401) setTimeout(() => Alert.alert('Hm...', 'Some conflicts with your account. Try to login.'), 0);
    if (error.request.status === 500) setTimeout(() => Alert.alert('Hm...', 'Looks like apple did not provide your name.'), 0);
  } finally {
    yield put(actions.turnOffSpinnerAction());
  }
}
...
function* appleLogin({ payload }) {
  try {
    ...
    yield put(actions.turnOnSpinnerAction());
    const { data } = yield call(api.post, endpoints.APPLE_LOGIN_ENDPOINT, { token });
